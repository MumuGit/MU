...
import com.mu.example.myapplication.okhttp3.internal.io.FileSystem;
import com.mu.example.myapplication.okhttp3.internal.platform.Platform;
 ...
 * string key and a fixed number  of values. Each key  must match  the regex <strong>{a-z0-9_-}{1,64}</strong>.
 * Values are byte sequences,accessible as steams or files.Each value must be between {@code 0} and {@code Integer.MAX_VALUE}
 * bytes in length.
 * <p>
 * <p>The cache stores its data in a directory on the filesystem.This directory must be exclusive to the cache; the cache may
 * delete or overwrite files from its directory.It is a error for multiple processes to use the same cache directory at the same
 * time.
 * <p>This cache limits the number of  bytes that it will store  on the filesystem.When the number of stored bytes exceed the
 * limit, the cache will remove entries in the background until the limit is satisfied.The limit is not strict: the cache may
 * temporarily exceed it while waiting for files to be deleted.The limit does not include filesystem overhead or the cache journal
 * so space-sensitive applications should set a conservative limit.
 * </p>
 * <p>
 * <p>Clients call {@link #edit} to create or update the values of an entry.An entry may have only one
...
public final class DiskLruCache implements Closeable, Flushable {
   ...
   */
    ...
    public static DiskLruCache create(FileSystem fileSystem, File directory, int appVersion, int valueCount,
        ...
        Executor executor = new ThreadPoolExecutor(0, 1, 60L,
                TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>(), Util.threadFactory("Okhttp DiskLruCache", true));
    ...
    private void readJournal() throws IOException {
        ...
        try {
            ...
            if (!source.exhausted()) {
                ...
                journalWriter = newJournalWritter();
            }
        } finally {
    ...
    private BufferedSink newJournalWritter() throws FileNotFoundException {
        Sink fileSink = fileSystem.appendingSink(journalFile);
        Sink faultHidingSink = new FaultHidingSink(fileSink) {
            @Override
            protected void onExecption(IOException e) {
